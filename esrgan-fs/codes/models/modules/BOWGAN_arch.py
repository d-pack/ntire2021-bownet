...
class BOWGAN(nn.Module):
    def __init__(self, in_nc, out_nc, nf, gc=32, nb=23):
        super(BOWGAN, self).__init__()
        ...
        self.ESRGAN = nn.Sequential(*[RRDB(nf=nf, gc=gc) for _ in range(nb)])

        self.upconv1 = nn.Conv2d(nf * 2, nf * 2, 3, 1, 1, bias=True)
        self.upconv2 = nn.Conv2d(nf * 2, nf * 2, 3, 1, 1, bias=True)
        self.HR_rrdb = RRDB(nf=nf * 2, gc=gc)
        self.conv_last = nn.Conv2d(nf * 2, out_nc, 3, 1, 1, bias=True)
    ...
    def forward(self, x):
        ...
        x_esr = self.ESRGAN(x_1)
        x_esr = x_esr + x_1
        x = torch.cat([x, x_esr], dim=1)
